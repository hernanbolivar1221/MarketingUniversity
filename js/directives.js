...
app.directive("totalDuration", function(){
    return {
        ...
        link : function(scope){
            ...
            if(value > 1){
                scope.time = Math.ceil(value) + " Semanas";
            }else{
                scope.time = (parseInt(scope.seconds) / 86400) + " Días";
            }
        },
        scope: {
            seconds : "@"
...
app.directive('bookmarklet2', function ($rootScope) {
        ...
        switch(scope.filter){
            ...
            case "completed":
                courses.completed().success(function(response){
                    scope.dataCourses = response;
                    $rootScope.completed_courses = scope.dataCourses;
                });
                break;
            case "available":
